//定义一页显示多少个
   		var pageSize = 5;
   		//查询总行数
   		$.post("article.do","method=queryArticleTagNum&&article_tag=html",function(rowCount){
    		//等我已经拿到返回值了  在去计算要分页的显示情况
    		$("#html_Pagination").pagination(rowCount, {  
		         callback: htmlPageCallback,  //回调函数  用来加载某一页的数据的Ajax的方法
    	...
    	function htmlPageCallback(pageIndex,jq){
	    	$.post("article.do",{"article_tag":"html","method":"queryArticleByTag","currentPage":pageIndex,"pageSize":pageSize},function(htmltaglist){
	    		//获取到数据之后先清除之前的内容
	    		$("#html_main").empty();
	    		$(htmltaglist).each(function(){
