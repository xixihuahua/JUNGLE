...
var pageSize = 5;
//查询总行数
$.post("article.do",{"method":"ArticleCount","user_id":$('#user_id')[0].value},function(rowCount){
	//等我已经拿到返回值了  在去计算要分页的显示情况
	$('#article_count').html(rowCount);
...
function PageCallback(pageIndex,jq){
	$.post("article.do",{"method":"queryArticlePage","user_id":$('#user_id')[0].value,"currentPage":pageIndex,"pageSize":pageSize},function(list){
		//获取到数据之后先清除之前的内容
		$("#main").empty();
		$(list).each(function(){
			...
			//未加编辑以及删除链接，后期可通过验证用户id与session的用户id决定是否有修改链接
				...
				+this.article_id+".html' ><b>"+this.article_title+"</b>  &nbsp;&nbsp;&nbsp;<i class='label label-warning'>"+this.article_tag+"</i></a></h4><a href='article-"
