//定义一页显示多少个
   		var pageSize = 5;
   		//查询总行数
   		$.post("article.do","method=queryArticleTagNum&&article_tag=javascript",function(rowCount){
    		//等我已经拿到返回值了  在去计算要分页的显示情况
    		$("#javascript_Pagination").pagination(rowCount, {  
		         callback: jsPageCallback,  //回调函数  用来加载某一页的数据的Ajax的方法
    	...
    	function jsPageCallback(pageIndex,jq){
	    	$.post("article.do",{"article_tag":"javascript","method":"queryArticleByTag","currentPage":pageIndex,"pageSize":pageSize},function(jstaglist){
	    		//获取到数据之后先清除之前的内容
	    		$("#javascript_main").empty();
	    		$(jstaglist).each(function(){
	    			var new_article_content = Sub_article_content(this.article_content);
	    			$("#javascript_main").append(
