   		...
   		var pageSize = 5;
   		//查询总行数
   		$.post("article.do","method=queryCommentArticleCount",function(rowCount){
    		//等我已经拿到返回值了  在去计算要分页的显示情况
    		$("#commentPagination").pagination(rowCount, {  
		         callback: commentPageCallback,  //回调函数  用来加载某一页的数据的Ajax的方法
    	...
    	function commentPageCallback(pageIndex,jq){
	    	$.post("article.do",{"method":"queryArticleCommentById","currentPage":pageIndex,"pageSize":pageSize},function(list){
	    		//获取到数据之后先清除之前的内容
	    		$("#commentmain").empty();
	    		$(list).each(function(){
	    			...
	    			//未加编辑以及删除链接，后期可通过验证用户id与session的用户id决定是否有修改链接
	    				...
	    				+this.article_tag+"</i></a></h4><a style='list-style: none;text-decoration:none;' href='article-"
	    				+this.article_id+".html' ><i class='has-success'>"+new_article_content+"</i></a><p><a href='article-"
	    				+this.article_id+".html' class='pull-right text-color-999'>编辑回复</a><p><a href='user-"
	    				+this.user_id+".html' class='aw-user-name'>"
