//定义一页显示多少个
   		var pageSize = 5;
   		//查询总行数
   		$.post("article.do","method=queryArticleTagNum&&article_tag=java",function(rowCount){
    		//等我已经拿到返回值了  在去计算要分页的显示情况
    		$("#java_Pagination").pagination(rowCount, {  
		         callback: javaPageCallback,  //回调函数  用来加载某一页的数据的Ajax的方法
    	...
    	function javaPageCallback(pageIndex,jq){
	    	$.post("article.do",{"method":"queryArticleByTag","article_tag":"java","currentPage":pageIndex,"pageSize":pageSize},function(javataglist){
	    		//获取到数据之后先清除之前的内容
	    		$("#java_main").empty();
	    		$(javataglist).each(function(){
	    			...
	    			$("#java_main").append(
	    	    			...
	    	    			//未加编辑以及删除链接，后期可通过验证用户id与session的用户id决定是否有修改链接
	    	    				...
	    	    				+this.article_id+".html' ><b>"
	    	    				+this.article_title+"</b>  &nbsp;&nbsp;&nbsp;<i class='label label-warning'>"
