//定义一页显示多少个
   		var pageSize = 5;
   		//查询总行数
   		$.post("article.do","method=queryArticleTagNum&&article_tag=web",function(rowCount){
    		//等我已经拿到返回值了  在去计算要分页的显示情况
    		$("#web_Pagination").pagination(rowCount, {  
		         callback: webPageCallback,  //回调函数  用来加载某一页的数据的Ajax的方法
    	...
    	function webPageCallback(pageIndex,jq){
	    	$.post("article.do",{"article_tag":"web","method":"queryArticleByTag","currentPage":pageIndex,"pageSize":pageSize},function(webtaglist){
	    		//获取到数据之后先清除之前的内容
	    		$("#web_main").empty();
	    		$(webtaglist).each(function(){
