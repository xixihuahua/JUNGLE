//定义一页显示多少个
   		var pageSize = 5;
   		//查询总行数
   		$.post("article.do","method=queryArticleTagNum&&article_tag=jquery",function(rowCount){
    		//等我已经拿到返回值了  在去计算要分页的显示情况
    		$("#jquery_Pagination").pagination(rowCount, {  
		         callback: jqPageCallback,  //回调函数  用来加载某一页的数据的Ajax的方法
    	...
    	function jqPageCallback(pageIndex,jq){
	    	$.post("article.do",{"article_tag":"jquery","method":"queryArticleByTag","currentPage":pageIndex,"pageSize":pageSize},function(jqtaglist){
	    		//获取到数据之后先清除之前的内容
	    		$("#jquery_main").empty();
	    		$(jqtaglist).each(function(){
